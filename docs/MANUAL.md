...
**ルール**: 述語の中で言及できるのは**その束縛変数だけ**です（他の引数を参照する真の依存型は未対応。§16）。
...
- `-> 戻り型` は省略可（省略時は `Unit`）。
- `! {効果, ...}` は省略可（省略時は効果なし＝純粋）。効果が1つなら `! Net` のように波括弧を省略可。
...
**検査ルール**: 関数本体が実際に起こす効果（呼び出し先の効果の合併）は、宣言した効果行の
**部分集合**でなければなりません。

- 本体の効果 ⊄ 宣言 → `effect-leak`（効果の隠蔽）エラー。
...
| code | 意味 | 典型的な2択 |
|---|---|---|
| `refine-conflict` | 精緻化の含意が成り立たない | (A) 要求をゆるめる / (B) ソースを強める（ガード等） |
...
type      := IDENT ('{' IDENT '|' pred '}')?        # IDENT は Int/Bool/Unit
...
- 精緻化は**単一の束縛変数 × 整数定数**のみ（線形整数算術の決定可能な断片）。引数間に依存する真の
  Π型（`b: Int{m | m < a}`）や本格的な SMT バックエンドは未対応。`implies` が反例を返すインターフェイスは、
  将来 Z3 等に差し替えても診断側を変えずに済むよう設計されています。
- ソルバは**この断片に対しては反例探索が完全**ですが、一般の決定手続きではありません。
- 効果は名前のラベル（＋オプションの精緻化パラメータ）。完全に再開可能（multi-shot）な効果ハンドラは
  `Exc`（例外）のみモデル化。
- 所有権は単純な linear-use 解析（直線コード＋`if`）。リージョン/ライフタイム推論なし。
- 型は `Int`/`Bool`/`Unit` のみ。ユーザー定義型・ジェネリクス・文字列・浮動小数点なし。
...
```proviso
# 関数・型・効果
fn name(x: Int, linear r: Int) -> Int{n | n >= 0} ! {IO, Net} { ... }
