...
- **依存精緻化**は動く（measure は `len` のみ）。`if len(xs) > 0` / `if i < len(xs)` の**ガードは理解され**
  （occurrence typing）、**リテラル配列の長さも静的に追跡**されます（`[10,20,30]` は `len == 3`）。よって
  範囲内添字は証明され実行時チェック不要、範囲外リテラル添字は静的な `bounds` エラーになります。ただし
  関数引数として渡った配列の長さは追跡されません（引数は gradual）。
...
- 評価器は**トランポリン化された CPS**なので、深い再帰（例 `sumto(100000)`）でも Python スタックは平坦の
  まま動きます。ハンドラ境界と resume だけは入れ子の駆動ループを使います（再帰段数ではなく、有効ハンドラ/
  resume の入れ子段数に比例）。
