...
`match` は `enum` 値を分解します。パターンはネスト可能で、網羅性はトップレベルで検査されます
(アームを増やすか `_` ワイルドカードを追加)。
...
- **エスケープする継続** — 捕捉された `k` は普通の値です。ハンドラがそれを返し、束縛し、後で
  ——それを生んだ `handle` の*外側*で——呼び出せます。そのような継続の呼び出しもグラデュアル
  呼び出しとして型検査を通ります。
...
状態は**グラデュアル**です。状態が確定できない束縛(注釈のないパラメータ、または分岐で食い違っ
てマージされたもの)は黙って受理されます。`@State` は**実行時に消去**されます——演算は普通の
関数なので、タイプステートのプログラムも動きます。遷移後に元の値を再利用できないよう、`linear`
と組み合わせてください。
...
- **タイプステートは静的のみ**(実行時の状態強制なし)。`@State` は消去される。
- コンストラクタフィールドの実行時契約は強制されない。
- `match` の網羅性はトップレベルで検査(ネストパターンは未対応)。
- 呼び出しは名前に対してのみ。ラムダリテラルや添字要素を即座に呼ぶことはできない——まず束縛
  する。
- LSP の差分編集は全文書同期として扱う。定義ジャンプは未実装。
- エスケープした継続はグラデュアルに型付けされる(呼び出せるが、精密な arrow 型は復元されない)。
...
文字列、マルチショット効果ハンドラ付きの一級関数、トランポリン化された評価器。テストスイートは
128 個の依存なしテストです。

将来のバージョンへ意図的に先送り: ネストパターンの網羅性、実行時のタイプステート強制、LSP の
差分同期と定義ジャンプ、エスケープした継続の精密(非グラデュアル)型。
